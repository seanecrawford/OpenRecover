...
class CarveResult:
    def __init__(self,start:int,end:int,sig:FileSignature,out_path:str,sha256:str,ok:bool,note:str=""):
        self.start=start; self.end=end; self.sig=sig; self.out_path=out_path; self.sha256=sha256; self.ok=ok; self.note=note
    ...
    def read(self,n:int)->bytes:
        if hasattr(self,"dev"):
            try: b=self.dev.read_at(self._pos,n)
                ...
                for skip in (1024*1024,128*1024,4096):
                    try: self._pos+=skip; b=self.dev.read_at(self._pos,n); break
...
class FileCarver:
    def __init__(self, source_path:str, output_dir:str, signatures:List[FileSignature]=None,
                 ...
                 fast_index:bool=False, max_bytes:int=0, min_size:int=256, progress_cb=None, deduplicate:bool=True):
    ...
    def _out(self,sig,start,length)->str:
        d=os.path.join(self.output_dir,sig.name); os.makedirs(d,exist_ok=True); return os.path.join(d,f"{sig.name}_@{start}_len{length}{sig.extensions[0]}")
    ...
    def scan(self):
        res=[]; read_total=0; r=Reader(self.source_path)
        try:
            total=getattr(r,"length",0) or 0; abs_off=0; tail=b""
            while True:
                if self.max_files and len(res)>=self.max_files: break
                if self.max_bytes and read_total>=self.max_bytes: break
                buf=r.read(self.chunk)
                if not buf: break
                read_total+=len(buf); data=tail+buf
                for sig in self.sigs:
                    i=0
                    while True:
                        idx=data.find(sig.header,i)
                        if idx==-1: break
                        rr=self._from_hdr(r, abs_off - len(tail) + idx, sig)
                        if rr:
                            res.append(rr); 
                            if self.max_files and len(res)>=self.max_files: break
                        i=idx+1
