...
@dataclass
class FileSignature:
    name:str; extensions:List[str]; header:bytes
    footer:Optional[bytes]=None; max_size:int=1024*1024*1024
    size_from_header:Optional[Callable[[BinaryIO,int],Optional[int]]]=None
    validate:Optional[Callable[[bytes],bool]]=None; header_adjust:int=0
def _png_validate(b:bytes)->bool: return b.endswith(b"\x00\x00\x00\x00IEND\xAE\x42\x60\x82")
def _ru32(f:BinaryIO,o:int): f.seek(o); b=f.read(4); return int.from_bytes(b,"big") if len(b)==4 else None
def _ru64(f:BinaryIO,o:int): f.seek(o); b=f.read(8); return int.from_bytes(b,"big") if len(b)==8 else None
def _iso_size(brands:List[bytes],max_scan:int=4*1024*1024*1024):
    def fn(f:BinaryIO,start:int)->Optional[int]:
        s=_ru32(f,start); 
        if not s or s<8: return None
        f.seek(start+4); t=f.read(4); 
        if t!=b"ftyp": return None
        major=f.read(4); ok=(len(major)==4 and major in brands)
        rem=s-16 if s>=16 else 0
        if rem>0 and not ok:
            samp=f.read(min(rem,64))
            for i in range(0,len(samp),4):
                if samp[i:i+4] in brands: ok=True; break
...
ALL_SIGNATURES=[JPEG,PNG,PDF,ZIP,WAV,MP4,HEIC]
